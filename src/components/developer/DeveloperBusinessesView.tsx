...
export default function DeveloperBusinessesView() {
  ...
  const filteredBusinesses = businesses.filter((biz) => {
    ...
    return (
      (biz.businessName && biz.businessName.toLowerCase().includes(s)) ||
      (biz.ownerName && biz.ownerName.toLowerCase().includes(s)) ||
      (biz.email && biz.email.toLowerCase().includes(s)) ||
      (biz.phone && biz.phone.toLowerCase().includes(s))
    );
  });
      ...
      <div className="max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800/80 pb-6">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1">
              Inspect tenant company profiles, logos, document volumes, owner contact information, and subscription states.
            </p>
          </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-[#0e131f] text-zinc-400 uppercase font-semibold border-b border-zinc-800 tracking-wider text-[10px]">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Business Logo</th>
                  ...
                  filteredBusinesses.map((biz) => {
                    ...
                    return (
                      <tr key={biz.userId} className="hover:bg-zinc-800/30 transition-colors">
                        ...
                        <td className="px-5 py-4">
                          {biz.hasLogo && biz.logoUrl ? (
                            <div
                              onClick={() => setSelectedLogo({ url: biz.logoUrl, name: biz.businessName })}
                              className="flex items-center gap-2 cursor-pointer group"
                              title="Click to view full logo"
                            >
                              <div className="w-12 h-8 rounded bg-white p-1 flex items-center justify-center border border-zinc-700 overflow-hidden group-hover:border-blue-400 transition-colors">
                                <img
                                  src={biz.logoUrl}
                                  alt="Logo"
                              ...
                              >
                                {biz.logoEnabled ? 'Active' : 'Hidden'}
                              </span>
                            </div>
                          ) : (
                            <span className="text-zinc-500 text-[11px] italic">No logo</span>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="font-semibold text-white text-xs">{biz.totalDocuments} total</div>
        ...
        {selectedLogo && (
          <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-[#121724] border border-zinc-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
              <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                <div className="flex items-center gap-2">
                  <ImageIcon className="w-4 h-4 text-purple-400" />
                  <span className="font-bold text-white text-sm">{selectedLogo.name} Logo</span>
                </div>
                <button
              ...
              <div className="p-6 bg-white rounded-xl flex items-center justify-center overflow-hidden">
                <img
              ...
              <div className="flex justify-end pt-1">
                <button
                  onClick={() => setSelectedLogo(null)}
                  className="h-8 px-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold rounded-xl cursor-pointer"
                >
                  Close
                </button>
              </div>
